...
def fit(length, fitness):
    ...
    for iter in iterations:
        ...
        for times in range(10):
          best_state, best_fitness = mlrose.random_hill_climb(problem, max_attempts = 10, max_iters = iter, restarts = 0, init_state = range(length))
        ...
        best_fitness = 0
        for times in range(10):
          best_state, best_fitness = mlrose.simulated_annealing(problem, schedule = mlrose.GeomDecay(), max_attempts = 10, max_iters = iter, init_state = range(length))
          best_fitness = min(best_fitness, best_fitness)
          #print(best_state)
